...
<table id="taula" class="tablesorter">
	...
	<?php for ($iLerro = 0; $iLerro < count($datuak); $iLerro++): ?>
		...
		<?php for ($iZutabe = 0; $iZutabe < count($datuak[$iLerro]); $iZutabe++): ?>
			<td title="<?php echo $goiburuak[$iZutabe]; ?>">
			<?php if ($iZutabe == 0): ?>
				<div class="izkutua"><?php echo $iLerro; ?></div>
			<?php endif; ?>
				<?php echo is_numeric($datuak[$iLerro][$iZutabe]) ? round($datuak[$iLerro][$iZutabe], 2) : $datuak[$iLerro][$iZutabe]; ?>
			</td>
		<?php endfor; ?>
		</tr>
	<?php endfor; ?>
